...
try:
    def slice_me():
        ...
        _rt._attr_(_dynapyt_ast_, 15, _rt._read_(_dynapyt_ast_, 14, lambda: ages), "append")(_rt._read_(_dynapyt_ast_, 16, lambda: new_highest_age))
        for age in _rt._gen_(_dynapyt_ast_, 20, _rt._read_(_dynapyt_ast_, 17, lambda: ages)):
            if _rt._enter_if_(_dynapyt_ast_, 19, _rt._read_(_dynapyt_ast_, 18, lambda: age) == 150):
                print("Bye!")
        return _rt._read_(_dynapyt_ast_, 21, lambda: ages) # slicing criterion
    
    _rt._read_(_dynapyt_ast_, 22, lambda: slice_me)()
